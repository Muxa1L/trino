...
public class VictoriaMetricsSplitManager
        implements ConnectorSplitManager
{
    static final long OFFSET_MILLIS = 1L;
    private final VictoriaMetricsClient victoriaMetricsClient;
    private final VictoriaMetricsClock victoriaMetricsClock;

    private final URI victoriaMetricsURI;

    @Inject
    public VictoriaMetricsSplitManager(VictoriaMetricsClient victoriaMetricsClient, VictoriaMetricsClock victoriaMetricsClock, VictoriaMetricsConnectorConfig config)
    {
        this.victoriaMetricsClient = requireNonNull(victoriaMetricsClient, "victoriaMetricsClient is null");
        this.victoriaMetricsClock = requireNonNull(victoriaMetricsClock, "victoriaMetricsClock is null");
        this.victoriaMetricsURI = config.getVictoriaMetricsURI();
    }
    ...
    {
        VictoriaMetricsTableHandle tableHandle = (VictoriaMetricsTableHandle) connectorTableHandle;
        VictoriaMetricsTable table = victoriaMetricsClient.getTable(tableHandle.schemaName(), tableHandle.tableName());
     ...
     * The times will be used in queries to VictoriaMetrics like: {@code http://localhost:9090/api/v1/query?query=up[21d]&time=1568229904.000"}
     *
     * <p>
     * NOTE: VictoriaMetrics instant query wants the duration and end time specified.
    ...
    protected static Optional<VictoriaMetricsPredicateTimeInfo> determinePredicateTimes(TupleDomain<ColumnHandle> predicate)
    {
        ...
        Optional<Set<ColumnHandle>> maybeOnlyPromColHandles = maybeKeySet.map(keySet -> keySet.stream()
                .filter(VictoriaMetricsColumnHandle.class::isInstance)
                .collect(Collectors.toSet()));
        Optional<Set<VictoriaMetricsColumnHandle>> maybeOnlyTimeStampColumnHandles = maybeOnlyPromColHandles.map(handles -> handles.stream()
                .map(VictoriaMetricsColumnHandle.class::cast)
                .filter(handle -> handle.columnType().equals(TIMESTAMP_COLUMN_TYPE))
                .filter(handle -> handle.columnName().equals("timestamp"))
                .collect(Collectors.toSet()));

        // below we have a set of ColumnHandle that are all VictoriaMetricsColumnHandle AND of TimestampType wrapped in Optional: maybeOnlyTimeStampColumnHandles
    ...
    private static Optional<VictoriaMetricsPredicateTimeInfo> processTimeDomains(Optional<Set<Domain>> maybeTimeDomains)
    {
        return maybeTimeDomains.map(timeDomains -> {
            VictoriaMetricsPredicateTimeInfo.Builder timeInfoBuilder = VictoriaMetricsPredicateTimeInfo.builder();
    ...
    private static class EffectiveLimits
    {
         ...
         */
        public EffectiveLimits(Instant defaultUpperBound, Duration maxQueryRangeDurationRequested, Optional<VictoriaMetricsPredicateTimeInfo> maybePredicateRange)
        {
