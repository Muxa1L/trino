...
public class VictoriaMetricsClient
{
    ...
    @Inject
    public VictoriaMetricsClient(VictoriaMetricsConnectorConfig config, JsonCodec<Map<String, Object>> metricCodec, TypeManager typeManager)
    {
        ...
        URI victoriaMetricsMetricsUri = getVictoriaMetricsMetricsURI(config.getVictoriaMetricsURI());
        tableSupplier = Suppliers.memoizeWithExpiration(
            () -> fetchMetrics(metricCodec, victoriaMetricsMetricsUri),
    ...
    private static URI getVictoriaMetricsMetricsURI(URI victoriaMetricsUri)
    {
    ...
    public Set<String> getTableNames(String schema)
    {
        ...
        if (schema.equals("default")) {
            ...
            //TODO VictoriaMetrics warnings (success|error|warning) could be handled separately
        ...
        throw new TrinoException(VICTORIA_METRICS_TABLES_METRICS_RETRIEVE_ERROR, "VictoriaMetrics did not return metrics list (table names): " + status);
    }

    @Nullable
    public VictoriaMetricsTable getTable(String schema, String tableName)
    {
        requireNonNull(schema, "schema is null");
    ...
    public ResponseBody fetchUri(URI uri)
    {
        ...
        try {
            ...
            throw new TrinoException(VICTORIA_METRICS_UNKNOWN_ERROR, "Bad response " + response.code() + " " + response.message());
        }
        catch (IOException e) {
            throw new TrinoException(VICTORIA_METRICS_UNKNOWN_ERROR, "Error reading metrics", e);
        }
    }
    ...
    private Optional<String> getBearerAuthInfoFromFile(Optional<File> bearerTokenFile)
    {
        return bearerTokenFile.map(tokenFileName -> {
            try {
                ...
                throw new TrinoException(VICTORIA_METRICS_UNKNOWN_ERROR, "Failed to read bearer token file: " + tokenFileName, e);
            }
        });
    }
